  ...
  return (
    <>
      ...
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">🇺🇸 Top US News</h2>
        <div className="space-y-6">
          {articles.map((article: any) => (
            ...
            >
              <summary className="article-summary list-none p-4 -m-4 rounded-lg transition-all duration-300 ease-out hover:bg-red-50">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0">
                    <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-red-100 text-red-800 text-sm font-bold group-open:bg-white group-open:text-red-600 transition-all">
                      {article.rank}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-600 text-base mb-4 leading-relaxed group-open:text-red-100">
                      {article.summary}
                    </p>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center text-sm text-gray-500 group-open:text-red-200">
                        <span className="font-medium">{article.source_name}</span>
                        <span className="mx-2">•</span>
                        <span>{new Date(article.published_date).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center text-red-600 font-medium text-sm group-open:text-white">
